...
void induce_sa(const String& s,
	...
	const std::vector<typename boost::range_difference<String>::type>& buckets, 
	Indexes& sa) {

	typedef typename std::make_unsigned<typename std::decay<decltype(s[0])>::type>::type ch_type;

	// sort L-type indexes
	std::vector<typename boost::range_difference<String>::type> poss(buckets.begin(), buckets.end() - 1);

	auto back_idx = boost::size(s) - 1;
	sa[poss[ch_type(s[back_idx])]++] = back_idx;
	...
	for (auto cur : sa | boost::adapotrs::reversed) {
		if (--cur >= 0 && t[cur]) {
			sa[--poss[ch_type(s[cur])]] = cur;
		}
	}
...
// s_begin:•¶Žš—ñ‚Ì‚Í‚¶‚ß sa_begin:SA‚Ì‚Í‚¶‚ß s‚Ì•¶Žš—ñ n:•¶Žš—ñ‚Ì’·‚³
template <typename CharIter, typename IndexIter>
void make(CharIter s_begin, IndexIter sa_begin, int n, int ch_max) {
	typedef typename std::make_unsigned<typename std::iterator_traits<CharIter>::value_type>::type ch_type;
	...
	auto num_lms = lms_end = lms_end - sa_begin;
